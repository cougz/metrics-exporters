"""Enhanced filesystem metrics collector with environment-aware strategies"""
import logging
import os
...
class FilesystemCollector(EnvironmentAwareCollector):
    ...
    def __init__(self, config=None):
        super().__init__(config, "filesystem", "Filesystem usage metrics with environment-aware collection")
    
    def collect(self) -> List[MetricValue]:
        """Collect filesystem metrics using environment-appropriate strategy"""
        try:
            # Use strategy to collect filesystem data
            result = self.collect_with_strategy("filesystem")
            
            if not result.is_success:
                logger.warning(f"Filesystem collection failed: {result.errors}")
                return []
            ...
            logger.debug(f"Collected {len(metrics)} filesystem metrics using {result.method_used}")
            return metrics
        
        except Exception as e:
            logger.error(f"Filesystem collection failed: {e}")
            return []